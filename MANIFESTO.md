...
HEWB v1.4 (2026-07-19) measures lead time against ten landmark EU non-renewals under this discipline, over an annual cutoff schedule from 2009, with a literature-volume feature (Tier 1: population-relative hazard-language prevalence in the scientific literature, Europe PMC) and an ECHA CLH-intention feature (Tier 2) in the tabular feature set. At k=10 the model ranked chlorpyrifos 132 months before its EU ban, a lower bound, since its earliest flag sits at the very first cutoff tested. Nine of ten landmarks flagged within the top 50; one (epoxiconazole) was never flagged before its ban, reported, not hidden, per §3's honesty-over-novelty principle. Two v1.3 corrections govern how much to trust those numbers. First, a data fix: the EU PPDB export had listed Maneb under mancozeb's CAS, so the benchmark had been measuring mancozeb against Maneb's 2017 withdrawal; corrected, mancozeb is measured against its real 2021 non-renewal, where Hazium ranks it in the top-20 from 2010, roughly nine years earlier. Second, a method fix: adding that one substance to the population swung several borderline landmarks' k=10 ranks under single-shuffle scoring while the deep cases held unmoved, exposing fold-assignment variance; out-of-fold scores are now averaged over repeated cross-validation, so lead-times are reproducible rather than an artifact of one split. Stabilised, the 2023-01-01 headline average precision is 0.226; the Tier 2 CLH-intention feature lifts it to 0.254, a modest, honestly-attributed in-funnel gain that SHAP confirms sits below the independent literature signal rather than dominating it. Both are an order of magnitude above the trivial baselines. A separate, orthogonal question, whether graph embeddings (Node2Vec-style structural representations) beat this tabular baseline, remains a documented negative (§11's V2 gate stays closed): the literature feature is tabular, adds no graph edges, and does not change the 29.2% graph-coverage ceiling that decided that question. Full methodology in `BENCHMARK_SCOPE.md`; full result in `DEV_LOG.md`.
