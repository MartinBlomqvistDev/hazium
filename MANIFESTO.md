...
HEWB v1.2 (2026-07-19) measured lead time against ten landmark EU non-renewals under this discipline, over an annual cutoff schedule from 2009, with a literature-volume feature (Tier 1: population-relative hazard-language prevalence in the scientific literature, Europe PMC) added to the tabular feature set. At k=10, the model ranked chlorpyrifos 132 months before its EU ban and clothianidin (a bee-toxicity neonicotinoid) 120 months before its ban — both lower bounds, since their earliest flags sit at the very first cutoff tested. Clothianidin's flag lands roughly 59 months *before* the EU's own first regulatory action on the substance (a December 2013 partial restriction), not merely before its later formal non-renewal. Nine of ten landmarks flagged within the top 50; one (epoxiconazole) was never flagged before its ban. This is reported, not hidden, per §3's honesty-over-novelty principle. The literature feature is a real, measured improvement over the v1.1 tabular-only baseline, not a redundant addition: four landmarks that missed the strict k=10 threshold under v1.1 now flag inside it, and the 2023-01-01 aggregate average precision rose from 0.240 to 0.264. A separate, orthogonal question — whether graph embeddings (Node2Vec-style structural representations) beat this tabular baseline — was re-verified empirically the same day and remains a documented negative (§11's V2 gate stays closed): the literature feature is tabular, adds no graph edges, and does not change the 29.2% graph-coverage ceiling that decided that question. Full methodology in `BENCHMARK_SCOPE.md`; full result in `DEV_LOG.md`.
