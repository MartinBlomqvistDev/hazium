        ...
        {missed.length > 0 && (
          <div className="mt-6 rounded-lg border border-status-critical/30 bg-status-critical/[0.06] p-4 text-sm">
            <p className="text-text-primary">
              <strong>Reported, not hidden:</strong>{" "}
              {missed.map((m) => m.name).join(", ")} never entered the top 50
              before its real action. Per the project&apos;s own honesty
              principle, every learned result is reported against its
              baseline and its misses, or it isn&apos;t published at all.
            </p>
          </div>
        )}
